 ...
 * [중요] 사용 중인 NTC 센서의 데이터시트 값으로 교체하거나, 정밀 교정을 통해 구해야 합니다.
 * ('f' 접미사를 제거하여 double 상수로 정의합니다. 아래 값은 실제 A, B, C값을 계산하여 수정 필요.)
 * ============================================================================ */
#define SH_A (1.126475580e-3)
#define SH_B (2.344937954e-4)
#define SH_C (0.8679075474e-7)
// (I_BIAS는 HTC에 설정된 값(예: 100uA)에 맞춰야 함)
...
/* ============================================================================
 * [신규] 온도 캘리브레이션 설정 (소프트웨어 보정)
 * ============================================================================ */
#define CALIB_TEMP_GAIN   (1.0)  // 보정 비율 (기본값 1.0). 캘리브레이션 후 수정 필요.
#define CALIB_TEMP_OFFSET (0.0)  // 보정 오프셋 (°C) (기본값 0.0). 캘리브레이션 후 수정 필요.
/* ============================================================================ */
...
static float ntc_voltage_to_celsius(float v_div_f)
{
  ...
  const double lnR = log(r_ntc); // logf() 대신 log() 사용 (double precision)
  const double invT = SH_A + (SH_B * lnR) + (SH_C * lnR * lnR * lnR);

  // 4. 켈빈 온도를 섭씨 온도로 변환 후 float로 반환
  // (invT가 0에 가까워지는 극단적인 경우 처리)
...
static void DSP_Send_Data_Frame(void)
{
  ...
  int n = snprintf(g_tx_line, RDV2_TX_BUFSZ,
                   "du1|du2|st|%lu|%lu|%.3f|%u|%u|",  // sampling_rate 소수점 3자리까지 (kS/s)
...
//  HAL_UART_Transmit(&huart3, (uint8_t*)g_tx_line, (uint16_t)n, 100);
  HAL_StatusTypeDef tx_status = HAL_UART_Transmit(&huart3, (uint8_t*)g_tx_line, (uint16_t)n, 100);
    ...
    if (tx_status == HAL_OK) {
        uint32_t tickstart = HAL_GetTick();
        // UART_FLAG_TC (Transmission Complete) 플래그가 설정(SET)될 때까지 대기합니다.
        while (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_TC) == RESET)
        {
            // 안전을 위한 타임아웃 처리 (예: 5ms)
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART3) {
    ...
    if (c == '\n' || (g_rx_len >= 4 && strstr(g_rx_line, "|end") != NULL)) {
      ...
      // ▼▼▼▼▼ [수정] 파서 직접 호출 대신 깃발 세우기 ▼▼▼▼▼
      // 3) 새 파서 호출 (설정 프레임 반영) <-- (제거)
      // DSP_Parse_Settings(line_buf); // <--- 이 줄을 주석 처리하거나 삭제합니다.

      // 3) 메인 루프가 처리하도록 버퍼에 복사하고 깃발(Flag) 설정
      if (g_settings_flag == 0) { // 이전 프레임 처리가 끝났을 때만 복사
